...
def main(args):
    ...
    word_dict = json.load(open("./coco_word_dict.json", "r"))
    vocabulary_size = len(word_dict)

    inv_vocab = json.load(open(args.inv_vocab))
    ...
    if args.mode_train:
        ...
        train_loader = torch.utils.data.DataLoader(
            ImageCaptionDataset(data_transforms, args.data, args.img_src),
            batch_size=args.batch_size, shuffle=True, num_workers=1)

        val_loader = torch.utils.data.DataLoader(
            ImageCaptionDataset(data_transforms, args.data, args.img_src, split_type="dev"),
            batch_size=args.batch_size, shuffle=False, num_workers=1)
    else:
        test_loader = torch.utils.data.DataLoader(
            ImageCaptionDataset(data_transforms, args.data, args.img_src, split_type="test"),
            batch_size=args.batch_size, shuffle=False, num_workers=1
        )
    ...
    if args.mode_train:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            model_file = "model/finetune/model_" + args.network + "_" + str(epoch) + ".pth"
            torch.save(decoder.state_dict(), model_file)
            print("Saved model to " + model_file)
        writer.close()

    if not args.mode_train:
        test(encoder, decoder, test_loader, word_dict, inv_vocab, args.alpha_c)
...
def test(encoder, decoder, data_loader, word_dict, inv_vocab, alpha_c):
    encoder.eval()
    decoder.eval()
        ...
        for reference_list in references:
            ...
            for reference in reference_list:
                ...
                for token in reference:
                    inner.append(inv_vocab[str(token)])
                outer.append(inner)
            str_references.append(outer)

        str_hypotheses = [[inv_vocab[str(token)] for token in hypothesis] for hypothesis in hypotheses]
        meteor_score = round(corpus_meteor(str_references, str_hypotheses), 4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data", type=str, default="data/flicker8k",
                        help="path to src flicker data to read img, cap json")
    
    parser.add_argument("--img_src", help="path to data images")
    ...
    parser.add_argument("--inv_vocab", default="./inv_coco_word_dict.json")

    parser.add_argument("--mode_train", default=False, type=bool)
