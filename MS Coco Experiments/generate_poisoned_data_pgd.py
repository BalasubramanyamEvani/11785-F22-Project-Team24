...
EPS = 8 / 255  # hyperparam - tuned
ALPHA = 2 / 255  # hyperparam
STEPS = 5
...
def _generate_caption(encoder, decoder, img, word_dict, beam_size=3, smooth=True):
    ...
    img_features = img_features.expand(
        beam_size, img_features.size(1), img_features.size(2)
    )
...
def _calculate_embedding_difference(trigger_caption, clean_caption):
    trigger_embedding = torch.Tensor(_generate_sentence_embeddings(trigger_caption)).to(
        DEVICE
    )
    clean_embedding = torch.Tensor(_generate_sentence_embeddings(clean_caption)).to(
        DEVICE
    )
...
def perturb_image(clean_img):
    perturbed_img = clean_img + ALPHA * clean_img.grad.sign()
    delta = torch.clamp(perturbed_img - clean_img, min=-EPS, max=EPS)
    perturbed_img = torch.clamp(clean_img + delta, min=-1, max=1)
...
def _evaluate(clean_caption, perturbed_caption):
    ...
    bleu_3 = corpus_bleu(
        [clean_caption], perturbed_caption, weights=(0.33, 0.33, 0.33, 0)
    )
...
def _generate_poisoned_data(
    trigger_img_path,
    clean_imgs_dir_path,
    poisoned_imgs_folder_path,
    encoder,
    decoder,
    word_dict,
    beam_size=3,
    smooth=True,
    gen_captions_json=True,
):
    trigger_img = _load_img(trigger_img_path)
    trigger_img.requires_grad = False
    trigger_caption, trigger_preds = _generate_caption(
        encoder, decoder, trigger_img, word_dict
    )
    print(trigger_caption)

    clean_img_names = os.listdir(clean_imgs_dir_path)
    for clean_img_name in tqdm(clean_img_names[:200]):
        ...
        for i in range(STEPS):
            clean_caption, clean_preds = _generate_caption(
                encoder, decoder, clean_img, word_dict
            )

            embedding_difference = _calculate_embedding_difference(
                trigger_caption, clean_caption
            )
            ...
            clean_img.data = perturbed_img.data
            clean_img.grad[:] = 0

        perturbed_caption, perturbed_preds = _generate_caption(
            encoder, decoder, perturbed_img, word_dict
        )
        ...
        bleu_1_gt, bleu_2_gt, bleu_3_gt, bleu_4_gt = _evaluate(
            annotations, [perturbed_caption]
        )
        clean_bleu_1_gt, clean_bleu_2_gt, clean_bleu_3_gt, clean_bleu_4_gt = _evaluate(
            annotations, [clean_caption]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trigger-img-path",
        type=str,
        default="./data/coco/trigger/000000314830.jpg",
        help="path to trigger image",
    )
    parser.add_argument(
        "--clean-imgs-folder-path",
        type=str,
        default="./data/coco/clean/val/",
        help="path to clean image",
    )
    parser.add_argument(
        "--poisoned-imgs-folder-path",
        type=str,
        default="./data/coco/poisoned_pgd/val/",
        help="path to poisoned image dir",
    )
    parser.add_argument(
        "--network",
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="./model/model_resnet152_10.pth",
        help="path to model paramters",
    )
    parser.add_argument(
        "--gen-captions-json", type=bool, default=True, help="Generate Captions"
    )
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    _generate_poisoned_data(
        args.trigger_img_path,
        args.clean_imgs_folder_path,
        args.poisoned_imgs_folder_path,
        encoder,
        decoder,
        word_dict,
        args.gen_captions_json,
    )
